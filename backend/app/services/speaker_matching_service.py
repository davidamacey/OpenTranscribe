...
from app.models.media import Speaker, SpeakerProfile, SpeakerCollection, SpeakerCollectionMember, SpeakerMatch
from app.services.speaker_embedding_service import SpeakerEmbeddingService
from app.services.opensearch_service import add_speaker_embedding, find_matching_speaker
...
class SpeakerMatchingService:
    ...
    def match_speaker_to_known_speakers(self, embedding: np.ndarray, user_id: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def match_speaker_to_profile(self, embedding: np.ndarray, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        profile = self.db.query(SpeakerProfile).filter(
    ...
    def create_speaker_profile(self, name: str, user_id: int, 
                             description: Optional[str] = None,
                             initial_embedding: Optional[np.ndarray] = None) -> SpeakerProfile:
        """
    ...
    def assign_speaker_to_profile(self, speaker_id: int, profile_id: int, 
                                confidence: Optional[float] = None) -> Speaker:
        """
    ...
    def process_speaker_segments(self, audio_path: str, media_file_id: int, 
                               user_id: int, segments: List[Dict[str, Any]], 
            ...
            if match:
                ...
                if aggregated_embedding is not None and aggregated_embedding.size > 0:
                    add_speaker_embedding(
                        ...
                        display_name=speaker.display_name if speaker.display_name else match['suggested_name'],
                ...
                if aggregated_embedding is not None and aggregated_embedding.size > 0:
                    ...
                    logger.info(f"Storing embedding for speaker {speaker_id}: length={len(embedding_list)}, first_few={embedding_list[:3]}")
                    add_speaker_embedding(
                        speaker_id=speaker_id,
                        user_id=user_id,
                        name=speaker.name,
    ...
    def add_speaker_to_collection(self, profile_id: int, collection_id: int) -> bool:
        ...
        try:
            ...
            existing = self.db.query(SpeakerCollectionMember).filter(
    ...
    def find_and_store_speaker_matches(self, new_speaker_id: int, embedding: np.ndarray, 
                                     user_id: int, threshold: float = 0.5) -> List[Dict[str, Any]]:
        """
        ...
        try:
                            ...
                            "filter": {
                                "bool": {
                                    "filter": [
                                        ...
                                        {"bool": {"must_not": {"term": {"speaker_id": new_speaker_id}}}}
                                    ]
                                }
                            }
                        }
            ...
            for hit in response["hits"]["hits"]:
                ...
                if score >= threshold:
                    ...
                    existing_match = self.db.query(SpeakerMatch).filter(
    ...
    def get_speaker_matches(self, speaker_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            speaker_matches = self.db.query(SpeakerMatch).filter(
            ...
            for match in speaker_matches:
                ...
                matched_speaker_id = match.speaker2_id if match.speaker1_id == speaker_id else match.speaker1_id
