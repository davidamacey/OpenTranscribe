...
export const Img = ({src, alt, style}) => <Img src={useBaseUrl(src)} alt={alt} style={style} />;
...
![OpenTranscribe gallery - AI-powered transcription and media management](/img/screenshots/gallery/gallery-overview.png)
...
- 🎧 **High-Accuracy Transcription** using WhisperX with 100+ language support
- 🌍 **Multilingual Support** with configurable source language and translation options
- 👥 **Automatic Speaker Identification** with voice fingerprinting and merge tools
- 🤖 **AI-Powered Summarization** with 12 output languages and customizable prompts
...
- **i18n** - Multi-language support (EN, ES, FR, DE, PT, ZH, JA)
...
OpenTranscribe is open-source software licensed under the GNU Affero General Public License v3.0 (AGPL-3.0). This license ensures that all users, including those accessing the software over a network, have access to the source code. See [LICENSE](https://github.com/davidamacey/OpenTranscribe/blob/master/LICENSE) for details.
