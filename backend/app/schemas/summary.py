...
class ActionItem(BaseModel):
    ...
    status: Optional[Literal["pending", "completed", "cancelled"]] = Field("pending", description="Current status")
...
class SummaryData(BaseModel):
    ...
    speakers: List[SpeakerInfo] = Field(..., description="Analysis of each speaker's contributions")
    content_sections: List[ContentSection] = Field(..., description="Time-based content breakdown")
...
class SummarySearchHit(BaseModel):
    ...
    speakers: List[SpeakerInfo] = Field(..., description="Speaker information")
    created_at: str = Field(..., description="Creation timestamp")
    provider: str = Field(..., description="LLM provider used")
    model: str = Field(..., description="Model used")
    highlights: Optional[Dict[str, List[str]]] = Field(None, description="Highlighted matching text")
...
class SummaryAnalyticsResponse(BaseModel):
    ...
    action_items_trend: List[Dict[str, Any]] = Field(..., description="Action item trends over time")
...
class SummaryTaskStatus(BaseModel):
    ...
    task_id: str = Field(..., description="Celery task ID")
    status: Literal["pending", "in_progress", "completed", "failed"] = Field(..., description="Task status")
