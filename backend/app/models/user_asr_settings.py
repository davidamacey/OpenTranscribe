"""
SQLAlchemy models for user ASR provider settings
"""
...
class UserASRSettings(Base):
    """
    Model for storing user-specific ASR provider configurations.

    Each user can have multiple ASR provider configurations. The active configuration
    is tracked via the UserSetting table with key 'active_asr_config_id'.
    ...
    __table_args__ = (
        ...
        Index("ix_user_asr_settings_user_provider", "user_id", "provider"),
    )
    ...
    def __repr__(self):
        return f"<UserASRSettings(user_id={self.user_id}, name={self.name}, provider={self.provider}, model={self.model_name})>"
