...
async function runTests() {
  ...
  if (!titleElement || titleElement.textContent !== mockFileData.filename) {
    throw new Error(
      `File title not displayed correctly. Expected: ${mockFileData.filename}, Got: ${titleElement?.textContent}`
    );
  }
  ...
  if (commentItems.length !== mockComments.length) {
    throw new Error(
      `Incorrect number of comments displayed. Expected: ${mockComments.length}, Got: ${commentItems.length}`
    );
  }
  ...
  if (updatedCommentItems.length !== mockComments.length + 1) {
    throw new Error(
      `New comment was not added. Expected: ${mockComments.length + 1}, Got: ${
        updatedCommentItems.length
      }`
    );
  }
  ...
  if (commentText !== testComment) {
    throw new Error(
      `New comment text doesn't match. Expected: ${testComment}, Got: ${commentText}`
    );
  }
  ...
  if (transcriptSegments.length !== mockFileData.transcript.segments.length) {
    throw new Error(
      `Incorrect number of transcript segments. Expected: ${mockFileData.transcript.segments.length}, Got: ${transcriptSegments.length}`
    );
  }
