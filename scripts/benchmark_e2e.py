...
def find_benchmark_task_id(r: redis.Redis, dispatch_time: float) -> str:
    """Find the benchmark Redis key whose dispatch_timestamp is closest to dispatch_time.

    The pipeline stores timing data under benchmark:{task_id} where task_id
    is generated by dispatch.py. This may differ from the active_task_id on
    the media file when reprocesses overlap. Finding the key by timestamp
    ...
    for key in r.scan_iter(match='benchmark:*'):
        ...
        ts_raw = r.hget(key, 'dispatch_timestamp')
        if not ts_raw:
            continue
    ...
    if best_delta < 30:  # within 30 seconds of our dispatch
        return best_key
    return ""
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Quick benchmark (3 iterations, compact output)
  python scripts/benchmark_e2e.py --file-uuid abc123

  # Detailed report with VRAM breakdown
  ...
  python scripts/benchmark_e2e.py --file-uuid abc123 --timeout 3600 --detailed
    ...
    print(f'Fetching file metadata for {args.file_uuid}...')
    file_meta = get_file_metadata(args.backend_url, args.file_uuid, token, verify=verify)
    if file_meta:
    ...
    for i in range(args.iterations):
        ...
        trigger_reprocess(args.backend_url, args.file_uuid, token, verify=verify)

        # Get task_id from DB for progress tracking
        task_id = get_task_id_for_file(args.file_uuid)
        if task_id:
        ...
        success = poll_task_completion(
            args.backend_url, args.file_uuid, token,
        ...
        # Find the benchmark task_id by matching dispatch timestamp in Redis.
        # This is more reliable than active_task_id which can differ from
        # the task_id used internally by the pipeline chain.
        resolved_id = find_benchmark_task_id(r, dispatch_time)
        if resolved_id:
            if resolved_id != task_id:
                print(f'  Resolved benchmark task: {resolved_id} (vs DB: {task_id})')
            task_id = resolved_id
        elif not task_id:
    ...
    if all_results:
        ...
        with open(args.output, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['iteration', 'task_id', 'wall_elapsed'] + stage_keys)
            for r_item in all_results:
                row = [r_item['iteration'], r_item['task_id'], f"{r_item['wall_elapsed']:.3f}"]
    ...
    # Print report
    if args.detailed:
        print_detailed_report(all_results, all_vram, file_meta, args.file_uuid)
    else:
