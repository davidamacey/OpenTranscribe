...
## PKI / Mutual TLS (mTLS) Authentication
...
# Requires a valid client certificate (.p12 file)
```

Client certificates are generated by `scripts/pki/` and stored in `scripts/pki/test-certs/clients/`. For production deployments, replace test certificates with your organization's CA-signed certificates.
...
| `/docs/` | Docs (port 8080) | Embedded documentation site |
| `/api/ws` | Backend WebSocket | Real-time notifications |
| `/api/` | Backend REST API | All API endpoints |
| `/flower/` | Flower (port 5555) | Celery task monitoring dashboard |
