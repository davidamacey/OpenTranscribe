    ...
    try:
        with session_scope() as db:
            ...
            if not files_to_process:
                return {"status": "success", "message": "No files need waveform generation", "processed": 0}
        ...
        for media_file in files_to_process:
            try:
                result = _generate_waveform_for_file(media_file.id, media_file.storage_path, media_file.filename)
                if result:
                    processed_count += 1
                    logger.info(f"Generated waveform for file {media_file.id}: {media_file.filename}")
                else:
                    error_count += 1
                    logger.warning(f"Failed to generate waveform for file {media_file.id}: {media_file.filename}")

            except Exception as e:
                error_count += 1
                logger.error(f"Error processing file {media_file.id}: {e}")

        logger.info(f"Waveform generation completed: {processed_count} processed, {error_count} errors")
        return {
            "status": "success",
