...
covers \texttt{large-v3-turbo} with \texttt{int8\_float16} quantisation
...
\paragraph{Results.}
Figure~\ref{fig:whisper-batch} and Table~\ref{tab:whisper-vram} summarise
the sweep.  The pattern closely mirrors the diarization embedding study:
throughput saturates early, but VRAM grows linearly.
...
{\footnotesize
  ...
  Whisper model footprint: stable $-$ baseline $=$ \SI{1332}{\mega\byte}.
  Raw CSV: \texttt{docs/whisper-vram-profile/a6000\_large-v3-turbo\_2026-04-26.csv}.}
\end{table}

\paragraph{Key findings.}
\begin{itemize}[nosep,leftmargin=*]
  \item \textbf{Throughput plateau at batch=8.}  Going from batch=2 to
    batch=8 recovers \SI{29}{\percent} of wall time (RTF $0.013 \to 0.009$).
    Batch sizes 8 through 32 are statistically identical (RTF $= 0.009$
    throughout).  This matches the diarization result where throughput
    plateaued at embedding batch 16.
  \item \textbf{Linear VRAM growth.}  Above the plateau, each 8-unit
    increment in batch size costs ${\sim}\SI{928}{\mega\byte}$ of
    additional peak VRAM (encoder activations + attention maps).
  \item \textbf{batch=32 wastes \SI{5568}{\mega\byte}.}  The faster-whisper
    default for large-memory GPUs is batch=32, yet batch=8 achieves
    identical throughput at \SI{5269}{\mega\byte} peak versus
    \SI{10837}{\mega\byte}.  The freed \SI{5.5}{\giga\byte} is enough
    headroom for five additional concurrent PyAnnote diarization
    pipelines at ${\sim}\SI{1}{\giga\byte}$ each.
\end{itemize}
...
\begin{center}
{\footnotesize
\begin{tabular}{@{}lR{1.4cm}R{1.4cm}@{}}
\toprule
\textbf{GPU class} & \textbf{Safe max batch} & \textbf{Peak (MB)} \\
\midrule
6\,GB   & 4  & 4341 \\
8\,GB   & 8  & 5269 \\
12\,GB+ & 16 & 7125 \\
...
The \textbf{recommended universal cap is batch=16}: identical speed to
batch=24/32, \SI{3.7}{\giga\byte} less peak VRAM, and safe on every GPU
that can run \texttt{large-v3-turbo}.  For 6\,GB and 8\,GB GPUs the
cap is tighter (4 and 8 respectively) due to the 80\% headroom
constraint.  4\,GB GPUs cannot run \texttt{large-v3-turbo} at any
batch size (minimum peak \SI{3893}{\mega\byte} exceeds the
$0.80 \times \SI{4096}{\mega\byte} = \SI{3277}{\mega\byte}$ gate);
the \texttt{medium} or \texttt{small} models are required.
